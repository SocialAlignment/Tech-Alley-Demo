...
> = ({ children, className, ...props }) => {
    ...
    // Fix: Target the Dashboard scroll container if it exists
    const containerRef = React.useRef<HTMLElement>(null)
    if (typeof document !== "undefined" && !containerRef.current) {
        containerRef.current = document.getElementById("hub-scroll-container") as HTMLElement
    }
...
>(({ style, className, transition, ...props }, ref) => {
    ...
    return (
        <motion.img
            ref={ref}
            className={cn(
                "relative z-10 size-auto max-h-full max-w-full object-contain rounded-2xl shadow-2xl",
                className
            )}
            style={{ scale, ...style }}
            {...props}
