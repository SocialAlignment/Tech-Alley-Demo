...
const Beam = ({
    ...
    return (
        <motion.div
            ...
            className={`absolute left-[var(--x)] top-0 [aspect-ratio:1/var(--aspect-ratio)] [background:var(--background)] [width:var(--width)] will-change-transform translate-z-0`}
...
export const WarpBackground: React.FC<WarpBackgroundProps> = ({
    ...
    const [beams, setBeams] = useState<{
        top: { x: number; delay: number; hue: number; ar: number }[];
        right: { x: number; delay: number; hue: number; ar: number }[];
        bottom: { x: number; delay: number; hue: number; ar: number }[];
        left: { x: number; delay: number; hue: number; ar: number }[];
    } | null>(null);
